...
**注意：**根据 map 创建记录时，`CreatedAt`字段不会自动生成。
...
db.Where("name = ?", "小瓜皮_1").First(&user)
// SELECT * FROM users WHERE name = '小瓜皮_1' ORDER BY id LIMIT 1;

// Get all matched records
db.Where("name <> ?", "小瓜皮_1").Find(&users)
// SELECT * FROM users WHERE name <> '小瓜皮_1';

// IN
db.Where("name IN ?", []string{"小瓜皮_1", "小瓜皮_2"}).Find(&users)
// SELECT * FROM users WHERE name IN ('小瓜皮_1','小瓜皮_2');

// LIKE
db.Where("name LIKE ?", "%小瓜皮%").Find(&users)
// SELECT * FROM users WHERE name LIKE '%小瓜皮%';

// AND
db.Where("name = ? AND age >= ?", "小瓜皮_1", "22").Find(&users)
// SELECT * FROM users WHERE name = '小瓜皮_1' AND age >= 22;
...
// SELECT * FROM users WHERE created_at BETWEEN '2000-01-01 00:00:00' AND '2022-01-08 00:00:00';
...
```go
// Struct
db.Where(&User{Name: "小瓜皮_1", Age: 20}).First(&user)
// SELECT * FROM users WHERE name = "小瓜皮_1" AND age = 20 ORDER BY id LIMIT 1;

// Map
db.Where(map[string]interface{}{"name": "小瓜皮_1", "age": 20}).Find(&users)
// SELECT * FROM users WHERE name = "小瓜皮_1" AND age = 20;
...
**注意：**当通过结构体进行查询时，GORM将会只通过非零值字段查询，这意味着如果你的字段值为`0`，`''`，`false`或者其他`零值`时，将不会被用于构建查询条件，例如：

```go
db.Where(&User{Name: "小瓜皮_1", Age: 0}).Find(&users)
//// SELECT * FROM users WHERE name = "小瓜皮_1";
```
...
**GORM Model定义 章节中的 默认值 小节有相同用法。**



#### 指定结构体查询字段

在使用struct进行搜索时，可以通过将相关字段名或表名传递给Where（），来指定在查询条件中使用结构中的哪些特定值，例如：

```go
var users []User
db.Where(&User{Name: "小瓜皮_1"}, "name", "Age").Find(&users)
// SELECT * FROM users WHERE name = "jinzhu" AND age = 0;

db.Where(&User{Name: "小瓜皮_1"}, "Age").Find(&users)
...
查询条件可以以与Where类似的方式内联到First和Find等方法中。作用与`Where`查询类似，当内联条件与多个[立即执行方法](https://www.liwenzhou.com/posts/Go/gorm_crud/#autoid-1-3-1)一起使用时, 内联条件不会传递给后面的立即执行方法。
...
// Plain SQL
db.Find(&user, "name = ?", "小瓜皮_1")
//// SELECT * FROM users WHERE name = "小瓜皮_1";

db.Find(&users, "name <> ? AND age > ?", "小瓜皮_1", 20)
//// SELECT * FROM users WHERE name <> "小瓜皮_1" AND age > 20;
...
//// SELECT * FROM users WHERE name <> "小瓜皮_1" LIMIT 1;

// Not In
db.Not("name", []string{"ji小瓜皮_1nzhu", "小瓜皮_2"}).Find(&users)
//// SELECT * FROM users WHERE name NOT IN ("小瓜皮_1", "小瓜皮_2");
...
// Plain SQL
db.Not("name = ?", "小瓜皮_1").First(&user)
//// SELECT * FROM users WHERE NOT(name = "小瓜皮_1");

// Struct
db.Not(User{Name: "小瓜皮_1"}).First(&user)
//// SELECT * FROM users WHERE name <> "小瓜皮_1";
```
...
// Struct
db.Where("name = '小瓜皮_1'").Or(User{Name: "小瓜皮_2"}).Find(&users)
//// SELECT * FROM users WHERE name = '小瓜皮_1' OR name = '小瓜皮_2';

// Map
db.Where("name = '小瓜皮_1'").Or(map[string]interface{}{"name": "小瓜皮_2"}).Find(&users)
//// SELECT * FROM users WHERE name = '小瓜皮_1' OR name = '小瓜皮_2';
```
...
GORM 允许在 `Table` 方法中通过 FROM 子句使用子查询，例如：
...
// Cancel limit condition with -1 （-1 取消 Limit 条件）
...
// Cancel offset condition with -1 (-1 取消 Offset 条件)
...
果没有找到记录，可以使用包含更多的属性的结构体创建记录，`Attrs` 不会被用于生成查询 SQL 。
...
`Scopes` Scope是建立在链式操作的基础之上的，允许指定常用的查询，可以在调用方法时引用这些查询：
...
db.Model(&User{}).Where("name = ?", "小瓜皮_1").Or("name = ?", "小瓜皮_2").Count(&count)
// SELECT count(1) FROM users WHERE name = '小瓜皮_1inzhu' OR name = '小瓜皮_2'

db.Model(&User{}).Where("name = ?", "小瓜皮_1").Count(&count)
// SELECT count(1) FROM users WHERE name = '小瓜皮_1'; (count)
...
Selecting distinct values from the model（从模型中选择不同的值）：
...
////  UPDATE `users` SET `created_at` = '2022-08-17 13:31:34', `updated_at` = '2022-08-17 13:32:00', `deleted_at` = NULL, `name` = '小瓜皮', `age` = 20, `active` = true  WHERE `users`.`deleted_at` IS NULL AND `users`.`id` = 1
```
...
db.Model(&user).Select("*").Update(User{Name: "小瓜皮", Role: "admin", Age: 0})

// Select 除 Role 外的所有字段（包括零值字段的所有字段）
db.Model(&user).Select("*").Omit("Role").Update(User{Name: "小瓜皮", Role: "admin", Age: 0})
```
...
```go
db.Model(&User{}).Update("name", "小瓜皮").Error // gorm.ErrMissingWhereClause

db.Model(&User{}).Where("1 = 1").Update("name", "小瓜皮")
// UPDATE users SET `name` = "小瓜皮" WHERE 1=1

db.Exec("UPDATE users SET name = ?", "小瓜皮")
// UPDATE users SET name = "小瓜皮"

db.Session(&gorm.Session{AllowGlobalUpdate: true}).Model(&User{}).Update("name", "小瓜皮")
// UPDATE users SET `name` = "小瓜皮"
```
...
**警告：**删除一条记录时，请确保主键字段有值，否则会触发 [批量 Delete](https://gorm.io/zh_CN/docs/delete.html#batch_delete)，GORM 会删除该 model 的所有记录。例如：
...
如果不想引入 `gorm.Model`，也可以这样启用软删除特性：
...
**注意**GORM通常猜测这种关系，就好像覆盖外键名称已经存在于所有者的类型中一样，我们需要在关系中指定外键。
...
您可以使用标记约束设置OnUpdate、OnDelete约束，它将在使用GORM迁移时创建，例如：
...
也可以在删除记录时通过 `Select` 来删除关联的记录，查看 [Delete with Select](https://gorm.io/zh_CN/docs/associations.html#delete_with_select) 获取详情。
...
**注意:** 对于 many2many 关联，GORM 在创建连接表引用之前，会先 upsert 关联。如果你想跳过关联的 upsert，你可以这样做：
...
**注意：**只有当删除记录的主键不为零时，关联才会被删除。GORM将使用这些主键作为删除选定关联的条件：
...
可以通过 `func(db *gorm.DB) *gorm.DB` 实现自定义预加载 SQL，例如：
...
GORM 会在自动迁移或建表时创建约束，请参阅 [约束](https://gorm.io/zh_CN/docs/constraints.html) 或 [数据库索引](https://gorm.io/zh_CN/docs/indexes.html) 以了解详情
...
查看 [通用接口](https://gorm.io/zh_CN/docs/generic_interface.html) 获取更多详情。
...
此外，还可以看看 [复合主键](https://gorm.io/zh_CN/docs/composite_primary_key.html)
...
可以通过将`autoCreateTime`标记设置为`false`来禁用时间戳跟踪，例如：
...
对于具有`UpdatedAt`字段的模型，如果其值为零，则该字段将设置为更新或创建记录的当前时间：
...
GORM 提供了 `Set`, `Get`, `InstanceSet`, `InstanceGet` 方法来允许用户传值给 [钩子](https://gorm.io/zh_CN/docs/hooks.html) 或其他方法
...
GORM 允许通过 `index`、`uniqueIndex` 标签创建索引，这些索引将在使用 GORM 进行[AutoMigrate 或 Createtable ](https://gorm.io/zh_CN/docs/migration.html)时创建。
...
如果使用嵌入结构创建共享复合索引，则不能指定索引名称，因为多次嵌入结构会导致数据库中的索引名称重复。

在这种情况下，可以使用索引标记`composite`，它表示复合索引的`id`。与原始规则一样，具有相同结构复合`id`的所有字段都放在同一个索引中。但改进之处在于，它允许最派生/嵌入的结构通过`NamingStrategy`生成索引名称。例如：
...
如果创建了`表Foo`，则复合索引的名称将为`dx_Foo_myname`。
...
复合指数的名称分别为`idx_bar0_myname`和`idx\u bar1_myname`。
...
一个字段接受多个`index`，即`uniqueIndex`标记，这些标记将在一个字段上创建多个索引。
